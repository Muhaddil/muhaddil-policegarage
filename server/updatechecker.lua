...
local resourceRepo = 'Muhaddil/muhaddil-policegarage'
    ...
    PerformHttpRequest(githubApiUrl, function(statusCode, response, headers)
        if statusCode == 200 then
            ...
            if data and data.tag_name then
                ...
                printWithColor('[muhaddil-policegarage] - Error: Invalid JSON structure.', '31') -- Red
            end
        else
            printWithColor('[muhaddil-policegarage] - Failed to fetch version data. Status code: ' .. statusCode, '31') -- Red
        end
    end, 'GET')
end
...
function displayVersionData()
    ...
    if versionData.latestVersion then
        if versionData.latestVersion ~= currentVersion then
            print('╭────────────────────────────────────────────────────╮')
            printCentered('[muhaddil-policegarage] - New Version Available', boxWidth, '34') -- Blue
            ...
            print('╭────────────────────────────────────────────────────╮')
            printWrapped('[muhaddil-policegarage] - Up-to-date', boxWidth, '32')  -- Green
            printWrapped('Current version: ' .. currentVersion, boxWidth, '32') -- Green
            print('╰────────────────────────────────────────────────────╯')
        end
    else
        printWithColor('[muhaddil-policegarage] - No version data available.', '31') -- Red
    end
end
