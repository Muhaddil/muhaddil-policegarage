...
function saveToLocalStorage() {
    ...
    localStorage.setItem(
        "policegarage_configs",
        JSON.stringify(vehicleConfigsData),
    );
}
...
function renderVehicleList() {
    ...
    container.innerHTML = filtered
        .map((vehicle) => {
            ...
            const isSelected =
                selectedVehicle && selectedVehicle.model === vehicle.model;
...
function selectVehicle(model) {
    ...
    if (vehicleConfigsData[model]) {
        ...
        if (typeof currentCustomization.livery === "number") {
            currentCustomization.livery = null;
        }
    } else {
        currentCustomization = {
...
function updatePreview() {
    ...
    favoriteBtn.classList.toggle(
        "active",
        favoritesData.includes(selectedVehicle.model),
    );
...
function renderColors() {
    ...
    container.innerHTML = selectedVehicle.colors
        .map((color, index) => {
...
function renderLiveries() {
    ...
    if (!selectedVehicle.liveries || selectedVehicle.liveries.length === 0) {
        container.innerHTML =
            '<p style="color: var(--text-secondary); font-size: 12px;">No hay liveries disponibles</p>';
        return;
    }

    container.innerHTML = selectedVehicle.liveries
        .map((livery, i) => {
            const isSelected =
                currentCustomization.livery &&
                currentCustomization.livery.type === livery.type &&
                currentCustomization.livery.index === livery.index;

            const label = livery.type === "mod" ? "Design" : "Livery";

            return `
            <div class="livery-option ${isSelected ? "selected" : ""}" 
                 data-type="${livery.type}" 
                 data-index="${livery.index}">
                ${label} ${livery.index + 1}
            </div>
        `;
        })
        .join("");
...
function renderExtras() {
    ...
    if (!selectedVehicle.extras || selectedVehicle.extras.length === 0) {
        container.innerHTML =
            '<p style="color: var(--text-secondary); font-size: 12px;">No hay extras disponibles</p>';
        return;
    }

    container.innerHTML = selectedVehicle.extras
        .map((extra) => {
...
function setupEventListeners() {
    ...
    document
        ...
        .addEventListener("click", resetCustomization);
}
